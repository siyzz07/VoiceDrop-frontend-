...
const HomePage = () => {
  ...
  const rooms = [
    {
      ...
      hosts: [
        { name: "Virat Kohli", color: "bg-blue-500" },
        { name: "Anushka Sharma", color: "bg-green-500" },
      ],
      participants: 8,
    },
    {
      ...
      hosts: [
        { name: "Elon Musk", color: "bg-red-500" },
        { name: "Sundar Pichai", color: "bg-yellow-500" },
      ],
      participants: 12,
    },
  ];
  ...
  return (
    <div>
    <div className={`${containerStyles} min-h-screen font-sans antialiased`}>
    {createRoom &&  <CreateRoomPopup popup={()=>closePopup}/>}
      ...
      <main className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-4 sm:px-6 lg:px-8 py-6">
        {rooms.map((room, index) => (
          <div
            key={index}
            className={`p-5 rounded-xl shadow-lg transition-all duration-300 transform hover:scale-105 ${cardStyles}`}
            role="button"
            tabIndex={0}
